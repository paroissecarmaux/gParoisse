...
- **V6.8.c — Intégration transverse** : **✅ COMPLET** (2026-09-13, voir `docs/V6.8-C-ANNUAIRE-INTEGRATION.md`). `requests.personId`/`intentions.personId`/`intentions.personnelId` résolus en priorité depuis `directory` (repli sur `people`/`personnel` pour compatibilité, jamais l'inverse, jamais de création automatique) via un nouveau helper central `resolveDirectoryEntity()`/`resolveDirectoryPerson()`/`resolveDirectoryPersonnel()`. Recherche globale, diagnostic d'intégrité (doublons potentiels people/personnel jamais fusionnés, structure des rôles) et KPI « Membres » → « Annuaire » du Tableau de bord adaptés. `directory` ajoutée à l'export/import JSON global. **Aucune donnée réécrite, aucun id modifié, `people`/`personnel` et leurs écrans intacts.** 137 tests (121 → 137). Aucun test manuel en navigateur (pas d'environnement disponible).
